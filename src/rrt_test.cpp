...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    pub_marker(&start_marker_msg, &start_marker_pub, x0.x, x0.y);
    pub_marker(&goal_marker_msg, &goal_marker_pub, xf.x, xf.y);

    // Publish all edges (after RRT is built)
    for (auto edge : T_out.edges)
    {
      pub_line(&line_msg, &line_pub, edge.at(0).x, edge.at(0).y, edge.at(1).x, edge.at(1).y);
    }
